...
def get_or_create(tg_effective_user):
    tg_profile, _ = TgProfile.objects.get_or_create(
        ...
        username=tg_effective_user.username,
...
def start_handler(update: Update, context: CallbackContext):
    ...
    main_keyboard = [[KeyboardButton(node.name) for node in MenuItem.objects.all()]]
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.regex(r'^(BTC|ETH|DOGE)$'), keyboard_button_handler))
